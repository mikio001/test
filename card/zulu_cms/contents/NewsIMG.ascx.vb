...
Partial Class zulu_cms_contents_NewsTopList
    ...
    Protected Sub NewsTopList_RenderHtml(ByVal w As System.Web.UI.HtmlTextWriter, ByVal dItem As Object) Handles NewsTopList.RenderHtml
        ...
        sql = "select  " & top & " itemID,title,modifyDate,imgUrl,refUrl from ZCMS_CONTENT where contentID=@contentID and siteID=@siteID and expireDate > GETDATE() " & WnewType & " order by itemID desc"
        ...
        Else
            ...
            While dr.Read
                ...
                w.Write(" <img data-src='holder.js/300x200' alt='300x200' src='" & imgUrl & "' style='width: 100%'>")
                w.Write("<a href=""" & refUrl & """ target=""" & Target & """>" & dr.GetString(1) & "</a>")
                w.Write("  <small>" & dr.GetDateTime(2).ToString("d MMM yyyy") & "</small>")
                w.Write("</blockquote>")
