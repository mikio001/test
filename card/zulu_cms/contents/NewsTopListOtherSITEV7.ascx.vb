...
Partial Class zulu_cms_contents_NewsTopList
    ...
    Protected Sub NewsTopList_RenderHtml(ByVal w As System.Web.UI.HtmlTextWriter, ByVal dItem As Object) Handles NewsTopList.RenderHtml
        ...
        Else
            ...
            While dr.Read
                ...
                w.Write("<li>")
                w.Write("  <blockquote>")
                w.Write("<a href=""" & refUrl & """ target=""" & Target & """>" & dr.GetString(1) & "</a>")
                w.Write("  <small>" & dr.GetDateTime(2).ToString("d MMM yyyy") & "<cite title='" & dr.GetString(4) & "'>" & dr.GetString(4) & "</cite></small>")
                w.Write("   </blockquote>")
                ...
                'w.Write("<li style='list-style-image: url(../resources/bullet.jpg);border-bottom-style: dotted; border-bottom-width: 1px;padding-top:5px;padding-bottom:10px; border-bottom-color: #C0C0C0'><table style='width: 100%; height: 60px;' ><tr><td valign='middle'><a href=""" & refUrl & """ target=""" & Target & """>" & dr.GetString(1) & "</a> |" & dr.GetString(4) & "  " & dr.GetDateTime(2).ToString("d MMM yyyy") & "</td></tr></table></li>")
            End While
    ...
    Function GetSQL(ByVal index As Integer, ByVal WnewType As String)
        ...
        sql = " select " & top & " itemID,title,modifyDate,refUrl, Description from vwZCMS_CONTENT_NEWSOTHER where  propsStatus=1   " & Where & WnewType & " "
