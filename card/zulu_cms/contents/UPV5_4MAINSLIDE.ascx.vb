...
Partial Class zulu_cms_contents_SlideCalendar
    ...
    Protected Sub NewsTopList_RenderHtml(ByVal w As System.Web.UI.HtmlTextWriter, ByVal dItem As Object) Handles PhotoSlider.RenderHtml
        ...
        While dr.Read
            ...
            'w.Write("  <img border='0' src='http://www.up.ac.th/" & imgUrl & "' alt='' height='280' title='" & dr.GetString(1).ToString & "' />")
            'If refUrl <> "" Then w.Write("</a>")
            'w.Write("</div>")
            w.Write("<DIV class='wrap carousel-item'>")

            w.Write("<DIV class='slide-caption  comp-" & loopRing(i) & "'><!-- switching between .comp-a, .comp-b, .comp-c, .comp-d rotates the bg color of the caption-->")

            w.Write("<H2><A onclick='_gaq.push(['_trackEvent', 'Carousel_1_title', 'OffsiteClick', '#', 4]);' ")
            w.Write("href='#'></A><a href='#'></a>" & dr("title") & "</H2>")
            'w.Write("<DIV class='slide-callout'>")
            'w.Write("<P><A onclick='_gaq.push(['_trackEvent', 'Carousel_1_subtitle', 'OffsiteClick', '#', 4]);' ")
            'w.Write("href='#'></A><a href='" & refUrl & "'></a>" & subtitle & "</P>")
            'w.Write("<P class='more'></P></DIV>")
            w.Write("</DIV>")
            w.Write("<DIV class='carousel-image'><A onclick='_gaq.push(['_trackEvent', 'Carousel_1_img', 'OffsiteClick', '#', 4]);' ")
            w.Write("  href = '" & refUrl & "' >")
            w.Write("<IMG width='522' height='348'  class=main-image alt='' ")
            w.Write("src='http://www.up.ac.th/" & imgUrl & "'></A>		 </DIV></DIV>")
