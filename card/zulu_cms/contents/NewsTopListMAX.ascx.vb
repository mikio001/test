...
Partial Class zulu_cms_contents_NewsTopList
    ...
    Protected Sub NewsTopList_RenderHtml(ByVal w As System.Web.UI.HtmlTextWriter, ByVal dItem As Object) Handles NewsTopList.RenderHtml
        ...
        sql = "select  " & top & " itemID,title,modifyDate,refUrl,subtitle from ZCMS_CONTENT where contentID=@contentID and siteID=@siteID "
        ...
        Else
            ...
            While dr.Read
                ...
                End If
                w.Write("<br>")
                w.Write(" <div class='col-md-1'>")
                w.Write("  <i class='icon-book icon-md'></i>")
                w.Write("  </div>")
                w.Write(" <div class='col-md-11'>")
                w.Write("    <h4><a href='" & refUrl & "'>""" & dr("title") & """</a></h4>")
                w.Write("    <p>" & dr("subtitle") & "</p>")
                w.Write("  </div>")
                w.Write(" </li>")

                ' w.Write("<div class=""bs-callout bs-callout-danger""><a href=""" & refUrl & """ target=""" & Target & """>" & dr.GetString(1) & "</a> <span class=""smallText"">" & dr.GetDateTime(2).ToString("d MMM yyyy") & "</span></div>")
            End While
    ...
    Private Function subtitle() As Integer
