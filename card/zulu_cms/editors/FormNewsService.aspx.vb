...
Partial Class zulu_cms_editors_NewEditer
    ...
    Private Sub Save_Click()
        ...
        Else
            ...
            db.Execute("update ZCMS_CONTENT set title=@title,contentBody=@html,createDate=GETDATE(),modifyDate=GETDATE(),modifyBy=@username,contentType=@contentType,status=0,refUrl=@refUrl,imgUrl=@refImg,subTitle=@subTitle where itemID=@newsID", db.NewParam("title", newsTitle.Text), db.NewParam("html", ASPxHtmlEditor1.Html), db.NewParam("username", Zulu.Security.Factory.UserProvider.GetCurrentUsername), db.NewParam("newsID", newsID), db.NewParam("contentType", cmbContentType.Value), db.NewParam("refUrl", url), db.NewParam("subTitle", newsSubTitle.Text), db.NewParam("refImg", urlImg))
        End If
            ...
            If Not String.IsNullOrEmpty(newsID) Then
                ...
                If dr.Read Then
                    ...
                    If Not dr.IsDBNull(4) Then
                        ...
                        txtRefUrl.Text = dr.GetString(4) '"<a target='_blank' href='" & ResolveClientUrl(UploadDirectory & dr.GetString(4)) & "'>" &  & "<a>"
                    End If

                    If Not dr.IsDBNull(5) Then
                        '    btnRefImg.Value = dr.GetString(5)
                        txtRefImg.Text = dr.GetString(5) '"<a target='_blank' href='" & ResolveClientUrl(UploadDirectory & ) & "'>" & dr.GetString(5) & "<a>"
                    End If
