<%@ Control Language="VB" AutoEventWireup="false" CodeFile="GalleryEditor.ascx.vb" Inherits="zulu_cms_editors_NewsEditor" %>
...
<script type="text/javascript">
    ...
    function showEditor(newsID) {
        window.location = "Editor.aspx?editor=GalleryForm&itemID=" + newsID + "&contentID=" + contetID;
    }
</script>
<dx:ASPxGridView ID="ASPxGridView1" runat="server" AutoGenerateColumns="False" 
    ...
    <Columns>
        ...
        <dx:GridViewDataImageColumn Caption="ภาพตัวอย่าง" VisibleIndex="2" FieldName="refUrl">
        ...
        <dx:GridViewDataDateColumn FieldName="modifyDate" VisibleIndex="4" 
            Caption="แก้ไขล่าสุด">
        </dx:GridViewDataDateColumn>
        <dx:GridViewDataTextColumn FieldName="modifyBy" VisibleIndex="5" 
            Caption="ผู้แก้ไข">
        </dx:GridViewDataTextColumn>
        <dx:GridViewDataDateColumn FieldName="createDate" VisibleIndex="6" 
            Caption="วันที่สร้าง">
        </dx:GridViewDataDateColumn>
...
<asp:SqlDataSource ID="SqlDataSource1" runat="server" 
    ...
    SelectCommand="SELECT [itemID], [title], [modifyDate], [modifyBy], [createDate], [createBy],[contentType],orderNo,refUrl FROM [ZCMS_CONTENT] WHERE ([contentID] = @contentID) and ([siteID]=@siteID)  ORDER BY [orderNo]" 
