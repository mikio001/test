...
Partial Class zulu_cms_editors_NewsAdd
    ...
    Private Sub Save_Click(ByVal sender As Object, ByVal e As EventArgs)
        ...
        Else
            ...
            db.Execute("update ZCMS_CONTENT set title=@title,contentBody=@html,createDate=GETDATE(),modifyDate=GETDATE(),modifyBy=@username,expireDate=@expireDate,contentType=@contentType,status=0,refUrl=@refUrl,imgUrl=@refImg,subTitle=@subTitle where itemID=@newsID", db.NewParam("title", newsTitle.Text), db.NewParam("html", FckEditor1.HTML), db.NewParam("username", Zulu.Security.Factory.UserProvider.GetCurrentUsername), db.NewParam("expireDate", newsExpireDate.Date), db.NewParam("newsID", newsID), db.NewParam("contentType", cmbContentType.Value), db.NewParam("refUrl", btnRefUrl.Text), db.NewParam("subTitle", newsSubTitle.Text), db.NewParam("refImg", btnRefImg.Text))
        End If
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not Page.IsCallback AndAlso Not Page.IsPostBack Then
            ...
            If Not String.IsNullOrEmpty(newsID) Then
                ...
                Dim dr = db.GetReader("select title,contentBody,expireDate,contentType,refUrl,imgUrl,subTitle from ZCMS_CONTENT where itemID=@itemID", db.NewParam("itemID", newsID))
                db.ThrowIfError()
                ...
                db.Close()
            End If
            Dim folder = Server.MapPath("~/FileUpload/" & ContentID())
            ASPxHtmlEditor1.SettingsImageUpload.UploadImageFolder = "~/FileUpload/" & ContentID()
        ...
        With EditorContainer.CancelButton
            ...
            If PageForm = "" Then
                .ClientSideEvents.Click = ("function(s,e){window.location='" & Me.ResolveClientUrl(("~/zulu_cms/Editor.aspx?editor=NewsEditor&contentID=" & Me.ContentID)) & "&SiteID=" & GetSiteID() & "';}")
            Else
