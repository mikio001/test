...
Partial Class zulu_cms_editors_NewEditer
    ...
    Private Sub Save_Click()
        ...
        If String.IsNullOrEmpty(newsID) Then
            ...
            db.Execute("insert into ZCMS_CONTENT(contentID,title,contentBody,createDate,createBy,modifyDate,modifyBy,siteID,editorID) values(@contentID,@title,@html,GETDATE(),@username,GETDATE(),@username,@siteID,'CONTENT')", db.NewParam("contentID", ContentID), db.NewParam("title", newsTitle.Text), db.NewParam("html", ASPxHtmlEditor1.Html), db.NewParam("username", Zulu.Security.Factory.UserProvider.GetCurrentUsername), db.NewParam("siteID", GetSiteID()))
        ...
        Else
            ...
            db.Execute("update ZCMS_CONTENT set title=@title,contentBody=@html,createDate=GETDATE(),modifyDate=GETDATE(),modifyBy=@username where itemID=@newsID", db.NewParam("title", newsTitle.Text), db.NewParam("html", ASPxHtmlEditor1.Html), db.NewParam("username", Zulu.Security.Factory.UserProvider.GetCurrentUsername), db.NewParam("newsID", newsID))
        End If
