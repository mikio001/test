...
Partial Class Main_ST2_Menu
    ...
    Sub UpPage()
        ...
        db.Execute("update MY_LISTMENU set ParentField=0 where ParentField is null")
        Dim index = db.GetValue("select ParentField from MY_LISTMENU where Menu_PK=" & a)
        Dim dtb = db.GetDataTable("select Menu_PK from MY_LISTMENU where ParentField=(select ParentField from MY_LISTMENU where Menu_PK=" & a & ") order by MenuIndex,Menu_PK")
        Dim myIndex = 0
        For i As Integer = 0 To dtb.Rows.Count - 1
            If dtb.Rows(i).Item("Menu_PK") = a Then
                ...
                End If
            End If
            db.Execute("update MY_LISTMENU set MenuIndex=" & i & " where Menu_PK=" & dtb.Rows(i).Item("Menu_PK"))
        Next
        Dim b = db.Execute("update MY_LISTMENU set MenuIndex=MenuIndex+1 where MenuIndex=" & myIndex - 1 & " and ParentField=(select ParentField from MY_LISTMENU where Menu_PK=" & a & ")")
        Dim err = db.GetException
        Dim c = db.Execute("update MY_LISTMENU set MenuIndex=" & myIndex & "-1 where Menu_PK=" & a)
