<%@ Page Title="" Language="VB" MasterPageFile="~/Main/DefaultPage/MasterPageAUTH.master" AutoEventWireup="false" CodeFile="Material.aspx.vb" Inherits="Main_COFFEE_ManageTable" %>
...
<asp:Content ID="Content3" ContentPlaceHolderID="ContentPlaceHolder1" Runat="Server">
     <%--   <button type="button" class="btn btn-primary " style="margin-bottom:10px; margin-right:14px;" onclick="$('#mdReport').modal('show');"> <span class="glyphicon glyphicon-print"></span> พิมพ์รายงาน</button>--%>
     <button type="button" class="btn btn-primary " style="margin-bottom:10px; margin-right:14px;" onclick="window.open('print/rptMaterial.aspx?type=' + 'IM' , '_blank')"> <span class="glyphicon glyphicon-print"></span> พิมพ์รายงานวัสดุIM</button>
      <button type="button" class="btn btn-primary " style="margin-bottom:10px; margin-right:14px;" onclick="window.open('print/rptMaterial.aspx?type=' + 'IS' , '_blank')"> <span class="glyphicon glyphicon-print"></span> พิมพ์รายงานวัสดุIS</button>
        ...
        <Columns>
            ...
            <dx:GridViewDataComboBoxColumn Caption="ประเภท" FieldName="TypeID" VisibleIndex="8">
            ...
            <dx:GridViewDataTextColumn Caption="สถานที่" FieldName="location" VisibleIndex="11">
            </dx:GridViewDataTextColumn>
          
            <dx:GridViewDataComboBoxColumn Caption="IM/IS" FieldName="IS_IM" VisibleIndex="7">
                <PropertiesComboBox DataSourceID="typeim_is" TextField="IM_IS" ValueField="IM_IS">
                </PropertiesComboBox>
            </dx:GridViewDataComboBoxColumn>
    ...
    <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:MAINDB %>" DeleteCommand="UPDATE REF_MATERIAL SET  MaterialStatus = N'D' WHERE (MaterialID = @MaterialID)" InsertCommand="INSERT INTO REF_MATERIAL(MaterialCode, MaterialName, Barcode, MinVolumn, TypeID, UnitID, Remark, CreateDate, CreateUser, BalanceVolumn, location, MaterialStatus, IS_IM) VALUES (@MaterialCode, @MaterialName, @Barcode, @MinVolumn, @TypeID, @UnitID, @Remark, GETDATE(), @CreateUser, 0, @location, N'N', @IS_IM)" SelectCommand="SELECT MaterialID, MaterialCode, MaterialName, Barcode, MinVolumn, TypeID, UnitID, Remark, CreateDate, CreateUser, LastDate, LastUser, BalanceVolumn, Price, location, MaterialStatus, IS_IM FROM REF_MATERIAL WHERE (MaterialStatus = N'N')" UpdateCommand="UPDATE REF_MATERIAL SET MaterialCode = @MaterialCode, MaterialName = @MaterialName, Barcode = @Barcode, MinVolumn = @MinVolumn, TypeID = @TypeID, UnitID = @UnitID, Remark = @Remark, LastDate = GETDATE(), LastUser = @LastUser, location = @location, IS_IM = @IS_IM WHERE (MaterialID = @MaterialID) AND (IS_IM = @IS_IM)">
        <DeleteParameters>
